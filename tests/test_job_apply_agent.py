...
class JobApplyAgentTests(unittest.TestCase):
    ...
    def test_automation_handoff_report_prioritizes_answers_and_stop_actions(self) -> None:
        ...
        self.assertIn("final-answer-intake-server", report["one_command_resume"])
        self.assertIn("--run-post-answer-pipeline", report["one_command_resume"])
        self.assertIn("--post-answer-apply", report["one_command_resume"])
        self.assertIn("--post-answer-live-check", report["one_command_resume"])
        ...
        self.assertIn("final-answer-intake", report["next_commands"][1])
        self.assertIn("final_answer_intake_template_latest.json", report["next_commands"][2])
        self.assertIn("post-answer-pipeline --final-answer-intake-json", report["next_commands"][3])
        self.assertIn("post-answer-pipeline --synthetic-final-answers --synthetic-rehearse-queue", report["next_commands"][4])
        self.assertIn("post-answer-pipeline --fail-on-not-ready", report["next_commands"][5])
        self.assertIn("post-answer-pipeline --apply --live-check", report["next_commands"][6])
        self.assertIn("--approve-high-risk", report["next_commands"][7])
        self.assertIn("critical_input_confirmed_updates_latest.json", report["next_commands"][7])
