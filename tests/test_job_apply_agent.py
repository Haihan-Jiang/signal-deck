...
class JobApplyAgentTests(unittest.TestCase):
    ...
    def test_final_answer_blocker_report_and_telegram_alert_exclude_answer_text(self) -> None:
        ...
        self.assertIn("final_answer_user_input_needed.xlsx", report["action_pack"]["user_input_xlsx_file"])
        self.assertIn("final-answer-user-input", report["action_pack"]["user_input_generate_command"])
        self.assertIn("final-answer-autopilot --reply-file", report["action_pack"]["user_input_xlsx_validate_command"])
        self.assertIn("final_answer_user_input_needed.xlsx", report["action_pack"]["user_input_xlsx_validate_command"])
        self.assertIn("--dry-run", report["action_pack"]["user_input_xlsx_validate_command"])
        self.assertIn("--apply --live-check --include-values", report["action_pack"]["user_input_xlsx_run_command"])
        self.assertIn("--open-browser --open-limit 100", report["action_pack"]["user_input_xlsx_open_command"])
        ...
        self.assertFalse(report["action_pack"]["sends_answer_text_to_telegram"])
        ...
        self.assertIn("user input Excel", markdown)
        self.assertIn("write user input file", markdown)
        self.assertIn("validate user input Excel", markdown)
        self.assertIn("run user input Excel", markdown)
        ...
        self.assertIn("watch user input Excel", markdown)
        self.assertIn("run user input Excel and open verified pages", markdown)
        ...
        self.assertIn("User input Excel:", alert)
        ...
        self.assertIn("\u90ae\u7f16\u662f[ZIP_CODE]", alert)
        self.assertIn("final-answer-reply --reply-file", alert)
        self.assertIn("--validate-only --fail-on-not-ready", alert)
        self.assertIn("--run-post-answer-pipeline --fail-on-not-ready", alert)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Post Answer Runbook", workbook_xml)
            self.assertIn("citizenship_status", sheet_text)
            self.assertIn("Manual final-submit review", sheet_text)
    ...
    def test_final_answer_autopilot_waits_for_unfilled_reply_template(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reply_path.write_text(
                "# comment with <fill> should not count\nzip_or_postal_code：<fill>\n",
                encoding="utf-8",
            )
            ...
            self.assertIn("goal-proof", json.dumps(report["final_audit_commands"]))
    ...
    def test_final_answer_autopilot_validates_filled_reply_without_storing_answer_text(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report_path = root / "autopilot.json"
            markdown_path = root / "autopilot.md"
            reply_path.write_text("zip_or_postal_code：98004\n", encoding="utf-8")
            template_path.write_text(json.dumps(template, ensure_ascii=True, indent=2), encoding="utf-8")
            unblockers_path.write_text(json.dumps(unblockers, ensure_ascii=True, indent=2), encoding="utf-8")
