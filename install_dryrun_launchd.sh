...
WRAPPER_PATH="$RUNTIME_ROOT/run_dryrun_daemon.sh"
...
chmod +x "$RUNTIME_ROOT/dryrun_recorder.py" "$RUNTIME_ROOT/live_experiment_signal.py" "$RUNTIME_ROOT/signal_engine.py" "$RUNTIME_ROOT/run_dryrun_job.sh" "$WRAPPER_PATH"
...
<dict>
  ...
  <dict>
    ...
    <key>SIGNAL_DECK_LOOP_INTERVAL</key>
    <string>$LOOP_INTERVAL</string>
  </dict>
...
echo "LoopInterval: ${LOOP_INTERVAL}s"
